                        ...
                        <div ref={carouselContainer} className='w-full h-[450px]   overflow-x-scroll scroll  scroll-smooth  no-scrollbar flex items-center gap-3'>
...

                            {
                                data?.map((item) => {
                                    ...
                                    // console.log(Type)
                                    return (
                                        <div key={item.id} onClick={() => router.push(`/Details/${item.media_type || endpoint}/${item.id}`)} className='bg-primary relative shadow-2xl border border-secondary rounded-lg min-w-44 max-h-[400px] min-h-[400px]   cursor-pointer hover:scale-105 ease-in-out duration-300' >
